  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? <Loader /> : filtered.length === 0 ? (
        <p className="text-center text-subtle py-10">No hay productos</p>
      ) : filtered.map((p) => (
        <button key={p.uuid} onClick={() => setDetail(p)} className="w-full text-left bg-surface rounded-2xl p-4 border border-line active:bg-surface-soft">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <p className="font-bold text-fg">{p.name}</p>
              <p className="text-xs text-muted">{p.category || "Sin categoría"} · Stock: {stockMap[p.uuid] || 0} u</p>
              <div className="mt-2 flex gap-4">
                <span className="text-sm"><span className="text-muted">Costo </span><b className="text-danger">{formatMoney(p.cost_price)}</b></span>
                <span className="text-sm"><span className="text-muted">Venta </span><b className="text-success">{formatMoney(p.unit_price)}</b></span>
              </div>
            </div>
        ...
        <Modal title="Nuevo producto" onClose={() => setShowAdd(false)}>
          ...
          <div>
            ...
            <select value={addForm.category} onChange={(e) => setAddForm({ ...addForm, category: e.target.value })}
              className="w-full px-3 py-2 border border-line rounded-xl bg-surface">
          ...
          />
          <p className="text-xs text-subtle">El stock inicial se carga como el primer fardo. Después podés sumar más.</p>
          <button onClick={addProduct} className="w-full py-3 bg-brand-solid text-white rounded-xl font-bold">Agregar</button>
        </Modal>
      )}
  ...
  return (
    <Modal title={product.name} onClose={onClose}>
      ...
      <FieldWithScan
        ...
        onScan={() => setScanning(true)}
      />
      <button onClick={saveFields} disabled={saving} className="w-full py-2.5 bg-brand-solid text-white rounded-xl font-bold">
        {saving ? "Guardando..." : "Guardar cambios"}
      </button>

      <BatchManager productUuid={product.uuid} productSource="products" unit="u" step="1" />

      <button onClick={darDeBaja} className="w-full py-2.5 bg-surface border border-danger text-danger rounded-xl font-semibold">
...
export function SearchBar({ value, onChange, count }) {
  return (
    <div className="relative">
      <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-subtle">search</span>
      <input value={value} onChange={(e) => onChange(e.target.value)} placeholder="Buscar..."
        className="w-full pl-10 pr-4 py-3 bg-surface border border-line rounded-xl focus:outline-none focus:border-brand" />
      {count != null && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-subtle">{count}</span>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <input type={text ? "text" : "number"} inputMode={text ? "text" : "decimal"} value={value}
        ...
        className="w-full px-3 py-2 border border-line rounded-xl focus:outline-none focus:border-brand" />
    </div>
  );
}
    ...
    <div>
      ...
      <div className="flex gap-2">
        <input type="text" inputMode="text" value={value} onChange={(e) => onChange(e.target.value)}
          className="flex-1 px-3 py-2 border border-line rounded-xl focus:outline-none focus:border-brand" />
        <button type="button" onClick={onScan}
          className="px-3 py-2 bg-brand-solid text-white rounded-xl flex items-center justify-center active:scale-95 transition-transform">
...
export function IconBtn({ icon, color, onClick }) {
  return (
    <button onClick={onClick} className="p-2 rounded-lg active:bg-surface-mute">
...
export function Fab({ onClick }) {
  return (
    <button onClick={onClick}
      className="fixed right-5 bottom-24 z-30 w-14 h-14 rounded-full bg-brand-solid text-white shadow-lg flex items-center justify-center active:scale-95 transition-transform">
...
export function Modal({ title, onClose, children }) {
  return (
    <div className="fixed inset-0 z-40 bg-black/50 flex items-end sm:items-center justify-center" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="w-full sm:max-w-md bg-surface rounded-t-3xl sm:rounded-3xl p-5 space-y-3 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between sticky -top-5 bg-surface pt-1 pb-2 -mt-1">
          <h2 className="text-lg font-bold text-fg">{title}</h2>
          <button onClick={onClose}><span className="material-symbols-outlined text-muted">close</span></button>
        </div>
        {children}
      </div>
    </div>
...
export function Loader() {
  return <div className="flex justify-center py-16 text-muted"><span className="material-symbols-outlined animate-spin text-3xl">progress_activity</span></div>;
}
export function Toast({ text }) {
  return <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 bg-fg text-white text-sm px-4 py-2 rounded-full shadow-lg">{text}</div>;
}
