  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? <Loader /> : filtered.length === 0 ? (
        <p className="text-center text-subtle py-10">No hay productos por peso</p>
      ) : filtered.map((p) => (
        <button key={p.uuid} onClick={() => setDetail(p)} className="w-full text-left bg-surface rounded-2xl p-4 border border-line active:bg-surface-soft">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              ...
              <p className="text-xs text-muted">{p.category || "Sin categoría"} · Stock: {formatNum(stockMap[p.uuid] || 0)} kg</p>
              <div className="mt-2 flex gap-4">
                <span className="text-sm"><span className="text-muted">Costo/kg </span><b className="text-danger">{formatMoney(p.cost_price_kg)}</b></span>
                <span className="text-sm"><span className="text-muted">Venta/kg </span><b className="text-success">{formatMoney(p.price_kg)}</b></span>
              </div>
            </div>
      ...
      {showAdd && (
        <Modal title="Nuevo producto por peso" onClose={() => setShowAdd(false)}>
          ...
          <p className="text-xs text-subtle">El stock inicial se carga como la primera tanda.</p>
          <button onClick={addItem} className="w-full py-3 bg-brand-solid text-white rounded-xl font-bold">Agregar</button>
        </Modal>
      )}
